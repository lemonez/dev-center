...
We have taken a different approach to how we store and access algorithms. The Entity Model Update project provides UUID APIs for algorithms, users, and hosted collections to decouple the algorithm from entity owners and entity names. Hosted data collections can now be assigned to the algorithm as default and accessed via `data://algo./default`. 

API routes are added for algorithms, users, and hosted collections and addressed by their UUIDs. These include calling algorithms, performing builds, and other CRUD operations that were previously only available through a combination of owner/algorithm name. 

CRUD operations now use UUIDS to assign algorithms to hosted collections by default. Users can also get a list of hosted data collections they own, including collections’ UUIDS.
To learn more about how to take advantage of this new feature see API documents located here: [add docs link]
